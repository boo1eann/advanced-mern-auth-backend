...
import { NotFoundExeption } from '../../common/utils/catch-errors';
...
export class SessionController {
	...
	public getAllSession = asyncHandler(async (req: Request, res: Response) => {
		...
		if (!userId) {
			throw new NotFoundExeption('User not authenticated');
		}
		const sessionId = req.sessionId;

		const { sessions } = await this.sessionService.getAllSession(userId?.toString());
	...
	public getSession = asyncHandler(async (req: Request, res: Response) => {
		...
		if (!sessionId) {
			throw new NotFoundExeption('Session ID not found. Please log in');
		}
